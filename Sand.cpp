...
void sandSystem::detonate(Vector2D loc, double power, double range) {
	/*for (int x = loc.x-range < 0 ? 0 : loc.x-range; x < (loc.x+range > SAND_SYSTEM_X ? SAND_SYSTEM_X : loc.x+range); x++) {
		for (int y = loc.y-range < 0 ? 0 : loc.y-range; y < (loc.y > SAND_SYSTEM_Y ? SAND_SYSTEM_Y : loc.y+range); y++) {
			double a = x - loc.x;	// TODO: Do these need to be doubles? x is an int, loc is an int, and these squared will be ints.
			double b = y - loc.y;
...
void sandSystem::affixSand(sandPart &i) {
	...
	i.vel.SetDM(2*PI-i.vel.GetDir(), 1);
	while (staticSand[(int)i.pos.x][(int)i.pos.y] != sf::Color::Transparent) {
		i.pos += i.vel;
	}
...
void sandSystem::render(sf::RenderWindow &window, Vector2D scrollPos) {
	...
	for (auto i : activeSandParts) {
		if (i.pos.x < SAND_SYSTEM_X && i.pos.y < SAND_SYSTEM_Y && !i.dead)
